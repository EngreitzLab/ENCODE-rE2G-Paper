...
bootstrapPerformance <- function(merged, pred_config, thresholds, n = 100, pos_col = "Regulated") {
  ...
  output <- bs_samples %>% 
    group_by(pred_uid) %>% 
    summarize(auprc_mean = mean(auprc, na.rm = TRUE),
              precision_mean = mean(precision, na.rm = TRUE),
              auprc_ci_lower = quantile(auprc, probs = 0.025, na.rm = TRUE),
              auprc_ci_upper = quantile(auprc, probs = 0.975, na.rm = TRUE),
              precision_ci_lower = quantile(precision, probs = 0.025, na.rm = TRUE),
              precision_ci_upper = quantile(precision, probs = 0.975, na.rm = TRUE))
...
run_bootstrap <- function(merged, pred_config, thresholds, pos_col = "Regulated") {
  ...
  bs_pairs <- merged %>% 
    select(name, all_of(pos_col)) %>% 
    distinct() %>% 
    sample_n(size = n_pairs, replace = TRUE)
  
  # return NULL if not both positives and ngatives in sampled pairs
  if (n_distinct(bs_pairs$Regulated) != 2) {
    return(NULL)
  }
  
  # get data for bootstrap sample
  bs_merged <- bs_pairs %>% 
    left_join(merged, by = c("name", pos_col)) %>% 
    select(all_of(colnames(merged)))
...
makePerfDistance <- function(merged_dist, pred_config, min_sensitivity = 0.7, pos_col = "Regulated",
  ...
  ggplot(perf_dist_bins, aes(x = x_label, y = AUPRC, fill = pred_name_long)) +
    ...
    geom_errorbar(aes(ymin = auprc_ci_lower, ymax = auprc_ci_upper),
                  position = position_dodge(width = 0.9), color = "black", width = 0.25) +
