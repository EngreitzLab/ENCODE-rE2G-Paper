...
# biosample = "encode_e2g_predictions_T-cell_ENCSR540VTN_thresholded_predictions.tsv.gz"
...
for(num_trait in 1:length(common_traits)){
  ...
  enr_value = prop_snps_gwas_celltype
  enr_traits_vec = c(enr_traits_vec, enr_value)
  cat("We are at trait:", num_trait)
}

outdff = data.frame("Trait" = common_traits,
                    "Recall" = enr_traits_vec)

write.table(outdff, file = paste0("/data/deyk/ENCODE/ENCODE_E2G_2024/processed/Recall_ENCODE_E2G_PoPS_top2/",
                                  biosample, "_", "ENR.txt"),
