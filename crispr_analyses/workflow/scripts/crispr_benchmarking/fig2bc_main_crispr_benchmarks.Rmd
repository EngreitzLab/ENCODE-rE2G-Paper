...
```{r savePRCPlotToFile}
# save to pdf
ggsave(filename = "results/manuscript/plots/fig2b_crispr_prc.pdf", height = 4.5, width = 12.5)
```
...
perf_dist <- computePerfSubsets(merged = merged_bins, pred_config = pred_config,
                                subset_col = "dist_bin", metric = "auprc", bs_iter = boot_iter)
...
```{r savePrefVsDistToFile}
# save plot to pdf
ggsave(filename = "results/manuscript/plots/fig2c_crispr_performance_distance.pdf", height = 4,
       width = 6)

# reformat performance table for supplementary table
perf_dist <- perf_dist %>% 
  left_join(select(pred_config, pred_uid, pred_name_long), by = c("id" = "pred_uid")) %>% 
  select(-c(subset_col, id)) %>% 
  dplyr::rename(distance_bin = subset, crispr_positives = pos, crispr_negatives = neg,
                predictor = pred_name_long) %>% 
  relocate(predictor, .before = metric)

# save performance table to file
write_tsv(perf_dist, file = "results/manuscript/tables/fig2c_performance_vs_distance.tsv")
```
...
write_tsv(pw_delta_auprc, file = "results/manuscript/tables/fig2b_pairwise_auprc_comparisons.tsv")
```
...
# save test results to file
write_tsv(pw_delta_prec, file = "results/manuscript/tables/fig2b_pairwise_precision_comparisons.tsv")
```
