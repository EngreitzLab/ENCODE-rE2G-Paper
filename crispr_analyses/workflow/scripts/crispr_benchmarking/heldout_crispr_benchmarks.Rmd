...
# compute weighted recall at threshold including confidence intervals
...
```{r}
# combine all weighted performance metrics into one table for supplementary table
weighted_perf <- bind_rows(weighted_auprc, weighted_precision, weighted_recall) %>% 
  mutate(metric = paste("weighted", metric, sep = "_")) %>% 
  select(-id) %>% 
  rename(predictor = pred_name_long) %>% 
  relocate(predictor, .before = metric)

# save performance table to file
write_tsv(weighted_perf, file = "results/manuscript/tables/crispr_heldout_performance.tsv")
```
...
# save plot to pdf
ggsave("results/manuscript/plots/fig2d_crispr_performance_heldout.pdf", height = 5.75,
       width = 4)
```
...
write_tsv(delta_perf_heldout,
          file = "results/manuscript/tables/crispr_heldout_pairwise_performance_comparisons.tsv")
```
...
# save plot to pdf
ggsave("results/manuscript/plots/extended_data_fig3g_crispr_performance_training_vs_heldout.pdf",
       height = 4, width = 15)
```
...
write_tsv(
  delta_perf_training_vs_heldout,
  file = "results/manuscript/tables/crispr_training_vs_heldout_pairwise_performance_comparisons.tsv")
```
