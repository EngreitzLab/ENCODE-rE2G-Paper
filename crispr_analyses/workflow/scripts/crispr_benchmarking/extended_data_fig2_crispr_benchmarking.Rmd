...
dir.create("results/manuscript/plots", showWarnings = FALSE, recursive = TRUE)
dir.create("results/manuscript/tables", showWarnings = FALSE, recursive = TRUE)
```
...
Compute and plot Pearson correlation coefficients between predictor scores and measured CRISPR
...
# save correlations with CRISPR effect sizes to table
write_tsv(pearson_cor, file = "results/manuscript/tables/extended_data_fig2f_crispr_correlation.tsv")
```
...
Calculate statistical between ENCODE-rE2G and corresponding ABC models.
```{r}
# function to test for statistical significance between two correlation coefficients taken from
# here: https://stats.stackexchange.com/questions/64152/are-two-pearson-correlation-coefficients-different
...
```{r include=FALSE}
# comparison between ENCODE-rE2G and ABC (DNase-only)
e2g <- filter(merged_plot, pred_uid == "ENCODE_rE2G.Score")
abc_dnase <- filter(merged_plot, pred_uid == "ABCdnase.ABC.Score")
comp1 <- cor.diff.test(x1 = e2g$pred_value, x2 = e2g$EffectSize,
                       y1 = abc_dnase$pred_value, y2 = abc_dnase$EffectSize,
                       method = "pearson")

# Comp 2 ENCODE-E2G_ext vs ABC (Full)
e2g_ext <- filter(merged_plot, pred_uid == "ENCODE_rE2Gext.Score")
abc_full <- filter(merged_plot, pred_uid == "ABCfull.ABC.Score")
comp2 <- cor.diff.test(x1 = e2g_ext$pred_value, x2 = e2g_ext$EffectSize,
                       y1 = abc_full$pred_value, y2 = abc_full$EffectSize,
                       method = "pearson")
```

Two-sided p-value ENCODE-rE2G vs. ABC^A=DNase, C=Average Hi-C^: `r comp1$p.value.twosided`
Two-sided p-value ENCODE-rE2G_Extended vs. ABC^Activity=DHS*H3K27ac, Contact=ENCODE Hi-C^: `r comp1$p.value.twosided`
