...
atac_models <- tibble(
  ...
  model = c("ENCODE_rE2GatacH3k27acHiC.Score", "ENCODE_rE2GatacH3k27ac.Score",
            "ENCODE_rE2GatacHiC.Score", "ENCODE_rE2Gatac.Score")
)
...
ggsave(filename = "results/manuscript/plots/fig4g_additional_models_bar.pdf", height = 6,
       width = 6)
```
...
ggsave(filename = "results/manuscript/plots/fig4f_additional_models_prc.pdf", height = 3.5,
       width = 8)
```
...
write_tsv(pw_delta_auprc,
          file = "results/manuscript/tables/fig4g_pairwise_auprc_comparisons.tsv")
```
