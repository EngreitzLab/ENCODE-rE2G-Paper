...
crispr_cols <- cols(ValidConnection = col_character())
...
crispr <- bind_rows(Training = crispr_training, `Held-out` = crispr_heldout, .id = "dataset")
...
p1 <- ggplot(filter(prop_classes, dataset != "Held-out (downsampled)"),
             aes(x = fct_rev(label), y = prop_pairs, fill = element_category)) +
  facet_wrap(~ "All CRISPR pairs") +
...
p2 <- ggplot(filter(prop_classes_reg, dataset != "Held-out (full)"),
  ...
  geom_bar(stat = "identity") +
  labs(fill = "Element category", y = "Proportion E-G pairs", x = "Dataset") +
  scale_fill_manual(values = cat_colors) +
  theme_bw() +
  theme(panel.grid = element_blank(), strip.background = element_blank(),
...
plot_grid(p1, p2, nrow = 1, rel_widths = c(0.4, 1), align = "h")
...
Make plots for experiments in indirect effects corrected benchmarks.
```{r}
# combined training and heldout crispr data into one table
use_datasets <- c("K562_DC_TAP", "WTC11_DC_TAP", "Klann", "Xie", "Gasperini2019")
crispr_filt <- filter(crispr, Dataset %in% use_datasets)
 
# calculate overall proportions between datasets
prop_classes <- crispr_filt %>% 
  count(dataset, element_category, name = "pairs", .drop = FALSE) %>% 
  group_by(dataset) %>% 
  mutate(total_pairs = sum(pairs),
         prop_pairs = pairs / total_pairs) %>% 
...
```{r bootstrapSignificancePrecision, eval=FALSE}
...
```{r precisionSignificanceValues, eval=FALSE}
...
```{r bootstrapSignificanceRecall, eval=FALSE}
# compute bootstrapped performance differences for all models between training and held-out data
...
```{r bootstrapSignificanceAUPRC, eval=FALSE}
# compute bootstrapped performance differences for all models between training and held-out data
...
Make effect sizes plot filtered to used datasets.
```{r, fig.height=4, fig.width=7}
used_refs <- c("Gasperini et al., 2019", "K562_DC_TAP_Seq", "WTC11_DC_TAP_Seq",
               "Klann et al., 2021", "Xie et al., 2019")

crispr_pos_refs <- crispr_positive_datasets %>% 
  filter(Reference %in% used_refs) %>% 
  mutate(Reference = factor(Reference, levels = used_refs, ordered = TRUE)) %>% 
  arrange(Reference) %>% 
  mutate(Source = fct_inorder(Source))

# plot effect size distribution per experimental dataset
ggplot(filter(crispr_pos_refs, feature == "Effect size"),
       aes(x = Source, y = value, color = Dataset)) +
  geom_jitter(width = 0.2) +
  geom_boxplot(width = 0.25, color = "black", fill = NA, outlier.shape = NA) +
  labs(y = "CRISPR effect size\n(percent change)", color = "Dataset type",
...
Make plot for used datasets
```{r, fig.height=6, fig.width=9}
# plot effect size distribution per experimental dataset
ggplot(filter(crispr_pos_refs, feature == "Effect size"),
       aes(x = element_category, y = value, color = element_category)) +
  facet_wrap(~Reference, scales = "free", drop = TRUE) +
  geom_jitter(width = 0.2) +
  geom_boxplot(width = 0.25, color = "black", fill = NA, outlier.shape = NA) +
  labs(y = "CRISPR effect size\n(percent change)", color = "Element category",
       title = "Effect sizes for CRISPR positives across element categories") +
