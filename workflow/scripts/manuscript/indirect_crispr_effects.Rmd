...
# extract CRISPR E-G pairs from merged data
benchmark_cols <- c("expressed", "PredictionCellType", "pred_elements", "pred_uid", "pred_id",
                    "pred_col", "pred_value", "Prediction")   
crispr_training <- distinct(select(merged_training, -any_of(benchmark_cols)))
crispr_heldout  <- distinct(select(merged_heldout,  -any_of(benchmark_cols)))
...
crispr <- bind_rows(training = crispr_training, heldout = crispr_heldout, .id = "dataset_type") %>% 
  ...
  mutate(dataset_plots = factor(dataset_plots, levels = datasets$dataset_plots))
```
...
# save plot to file 
ggsave(file = "results/manuscript/plots/crispr_direct_effects_rate.pdf", height = 4, width = 11)
```
...
# save plot to file 
ggsave(file = "results/manuscript/plots/crispr_direct_effects_rate_log_space.pdf", height = 4, width = 11)
```
...
```{r}
# extract crispr data for dataset with estimated indirect rates
crispr_datasets <- unique(na.omit(crispr$indirect_effects_dataset))
names(crispr_datasets) <- crispr_datasets
crispr_indirect <- filter(crispr, indirect_effects_dataset %in% crispr_datasets)
...
dist_direct_probs <- lapply(crispr_datasets, FUN = calculate_dist_direct_gt_prob,
                            direct_effect_models = direct_effect_models, trans_rates = trans_rates,
                            prob = 0.9)
...
crispr_indirect <- crispr_indirect %>% 
...
ggplot(crispr_indirect, aes(x = dist_to_tss / 1e3, y = direct_vs_indirect_negative,
                            color = dataset_plots)) +
  geom_line() +
  geom_point(data = filter(crispr_indirect, Regulated == TRUE), size = 1.75) +
...
## Enrichment for negative effects among likley direct pairs
```{r}
...
crispr_indirect_hits <- filter(crispr_indirect, Significant == TRUE)

# plot CRISPR effect size of likely direct vs. likely indirect effects
ggplot(crispr_indirect_hits, aes(x = dataset_plots, y = EffectSize, color = fct_rev(p90),
...
crispr_indirect_hits <- crispr_indirect_hits %>% 
  mutate(log2_fc_effect_size = log2(1 + EffectSize))

# plot CRISPR effect size of likely direct vs. likely indirect effects
ggplot(crispr_indirect_hits, aes(x = dataset_plots, y = log2_fc_effect_size, color = fct_rev(p90),
                                 group = interaction(dataset_plots, fct_rev(p90)))) +
  geom_hline(yintercept = 0, linetype = "dashed", linewidth = 0.5) +
  geom_point(alpha = 0.5, position = position_jitterdodge(jitter.width = 0.3)) +
  geom_boxplot(color = "black", fill = NA, outlier.shape = NA) +
